    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="学分类型" prop="creditType">
          <treeselect :disable-branch-nodes="true" @select="node => change(node)" v-model="form.creditType" :options="typeOptions" :normalizer="normalizer" placeholder="选择学分类型" disabled/>
        </el-form-item>
        <el-form-item label="学分值" prop="credit">
          <el-input v-model="form.credit" placeholder="学分值" disabled/>
        </el-form-item>
        <el-form-item label="客观学分" prop="credit">
          <el-input v-model="applyForm.objectiveCredit" placeholder="学分值"/>
        </el-form-item>
        <el-form-item label="申请附件" prop="requestFile">
          <file-upload :is-read="true" :isShowTip="false" v-model="form.requestFile"/>
        </el-form-item>
        <el-form-item label="审批备注">
          <el-input v-model="applyForm.remarks" placeholder="审批备注"/>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm(true)">通 过</el-button>
        <el-button type="danger" @click="submitForm(false)">打 回</el-button>
...
<script>
import {getCredit, delCredit} from "@/api/credit/credit";
import {listType, } from "@/api/credit/type";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";
import Cookies from "js-cookie";
import {approvalCredit, listCredit} from "@/api/credit/approval";
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            this.applyForm.applyId = this.form.id
            if (flag){
              this.applyForm.result = 1
              approvalCredit(this.applyForm).then(res => {
